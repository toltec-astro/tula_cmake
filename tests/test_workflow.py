...
from tula_cmake.workflow import BuildWorkflow, conan_command

if TYPE_CHECKING:
    import pytest


def test_conan_command_prefers_current_python_environment(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    python = tmp_path / "bin" / "python"
    conan = python.with_name("conan")
    conan.parent.mkdir()
    conan.touch()
    monkeypatch.setattr(sys, "executable", str(python))
    monkeypatch.setattr("tula_cmake.workflow.shutil.which", lambda _: "/usr/bin/conan")

    assert conan_command() == (str(conan),)
...
def test_macos_profile_pins_and_validates_llvm_20() -> None:
    profile = (profiles_dir() / "macos-brew-llvm-debug").read_text()

    assert '["brew", "--prefix", "llvm@20"]' in profile
    assert 'version.major != "20"' in profile
